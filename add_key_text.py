...
def get_dataframe_from_excel():
    ...
    # pd.to_datetime将'时间'列转为datetime类型
...
def main():
    ...
    # 选中数据框中的"总价"列，使用 sum() 函数计算"总价"列的和，使用 int() 函数求整
    total_sales = int(df_selection["总价"].sum())
    # 选中数据框中的"评分"列，使用 mean() 函数计算"评分"列的平均值，使用 round() 函数
    # 四舍五入，保留一位小数
    average_rating = round(df_selection["评分"].mean(), 1)
    # 对刚刚的结果再次四舍五入，只保留整数，并使用 int() 函数，表示就要整数，增加代码的可读性
    star_rating_string = ":star:" * int(round(average_rating, 0))
    # 选中数据框中的"总价"列，使用 mean() 函数计算"总价"列的平均值，使用 round() 函数四舍五入。
    # 保留两位小数
